...
The Category Master is optional but improves Engine 1's item-level matching.
"""
import os
import sys
import tempfile
...
def run_engine(gfc_path: str, master_path: str | None, boq_path: str | None) -> pd.DataFrame:
    ...
    results = process_gfc(gfc_path, budget, master, run_match=run_match)
...
if st.button("▶  Run Mapping", type="primary"):
    ...
    try:
        with st.spinner("Classifying line items…"):
            result = run_engine(gfc_path, master_path, boq_path)
    finally:
